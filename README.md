...
- **支持周期切换：**
  - 日 K、周 K、月 K（长期趋势）
  - 60 分钟 K、30 分钟 K、15 分钟 K、5 分钟 K、1 分钟 K（日内交易）
- **技术指标：**
  - MA5/10/20 均线（可单独开关）
  - VOL 成交量柱状图（红涨绿跌）
- **交互功能：**
...
### 获取 K 线数据（v1.2.0 新增）
```
GET /api/kline?symbol=600519&period=day&market=auto
```

**参数说明：**
- `symbol` - 股票代码（必填），如 `600519`、`HK0700`、`USAAPL`
- `period` - K 线周期（可选，默认 `day`）
  - `day` - 日 K
  - `week` - 周 K
  - `month` - 月 K
  - `60min` - 60 分钟 K
...
2. 点击股票卡片右上角的"📊 K 线"按钮
3. 在弹出的 K 线图窗口中：
   - **切换周期：** 点击顶部周期按钮（日 K/周 K/月 K/60 分钟等）
   - **查看指标：** 勾选/取消 MA5/10/20 均线开关
   - **缩放：** 使用鼠标滚轮
   - **拖拽：** 按住鼠标左键拖动查看历史数据
   - **查看详情：** 移动鼠标显示十字光标和 OHLC 数值
4. 点击右上角"×"或按 ESC 键关闭 K 线图
